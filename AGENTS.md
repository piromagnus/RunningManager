...
- Real dataset lives under `data/`. The useful per-activity datasets are committed (force-added past `.gitignore`): `data/timeseries/`, `data/metrics_ts/`, `data/hr_zones/`, `data/laps/`, `data/raw/`, `data/speed_profil/` — these are inputs for the app charts, benchmarks, and performance prediction. Still excluded: `data/exp_perf_predictions/` (large experiment output), `data/tokens*` and `.env` (secrets). Since those dirs remain listed in `.gitignore`, brand-new files added to them later are ignored by default — use `git add -f` to commit additional ones.
- `.env` holds `ENCRYPTION_KEY` (needed to decrypt `data/tokens.csv`) plus Strava keys; it exists on the VM disk only. The app runs fine without it (Strava sync just won't authenticate).
