...
| `trail_digital_twin_pipeline.py` | Configurable trail digital-twin fitting/report pipeline; cohorts include `runTrailOver20Min`; LOO caps/`loo_include`; HR QC in `segment_qc`; `software_versions.json`; Stage-3 ablation protocols `reoptimize` (default) vs `frozen`; ablation includes TRIMP→linear progress fatigue substitution |
