...
| Stage 3 | Add fixed linear HRR speed ratio and select decayed or cumulative acute TRIMP fatigue | t = d 3600 f_GAP / (VMA alpha f_alt f_REDI E(HRR) F(U)), U in {D, C} |
| Full regression comparator | OLS log-time diagnostic with E6 decayed TRIMP or E6bis cumulative TRIMP | log(t) = beta0 + beta_d log(d) + beta_g log(f_GAP) + ... |
