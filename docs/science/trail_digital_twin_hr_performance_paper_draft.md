...
| LUT 30k, Grésivaudan, Échappée Belle, Passerelles, Rome | Prospective constant-HRR hold-outs (preregistered) |
...
\(\mathrm{HRR}_{\mathrm{ref}}=0.88\) is the modeled **HRR at flat VMA effort** (\(E=1\)); `hrr_max_factor=1.20` allows \(E>1\) when HRR exceeds ref (short supra-VMA intervals). For these races, selected and observed mean HRR are all ≤0.88, so opening the ceiling does **not** change Table 5 point predictions—it matters for segment reconstruction when HRR spikes above ref. On the four trail races, constant observed-mean HRR yields a consistent ~13–19 min slow bias (|Δ| MAE ≈ 16.6 min) and is the only constant-HRR mode that stays well-behaved on Échappée Belle. Duration-feasible prospective is closer on Grésivaudan (+6.7) but still optimistic on the ultra (−73). A follow-on **segment-family adaptive** recipe (journal `journal_segment_hrr_adaptive.md`), ranked by **MAPE** and treating faster-than-actual predictions as acceptable, uses a short-race mid(PL, HRR_ref) push plus empirical duration blend on ultras (**H13b**; trail MAPE ≈ 2.1% vs ≈ 7.5% constant feasible). Rome remains **out of scope** (R5). Profiles: LUT/Grésivaudan = `race_pacing`+GPX altitude; Échappée/Passerelles = executed activity GPS.
...
| Prospective LUT / Passerelles / Grésivaudan / Échappée / Rome + bands (R9) | Table 5 |
